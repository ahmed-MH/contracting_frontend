...
export default function RoomsSection({ contract }: Props) {
    ...
    const handleRemove = async (roomId: number, roomName: string) => {
        const ok = await confirm({
            title: 'Remove contract room',
            description: `Remove "${roomName}" from this contract? Related data will also be deleted.`,
            confirmLabel: 'Remove',
            variant: 'danger',
        });
        ...
        <div id="rooms" className="scroll-mt-36 bg-brand-light shadow-sm ring-1 ring-brand-mint sm:rounded-xl p-6">
            ...
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-brand-slate/20">
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-base font-semibold text-brand-navy">{t('auto.features.contracts.details.sections.roomssection.151ee688', { defaultValue: 'Contract rooms' })}</h2>
            ...
            {rooms.length === 0 && (
                <div className="rounded-xl bg-brand-light border border-dashed border-brand-slate/20 p-12 text-center">
                    ...
                    <p className="text-brand-slate text-sm">{t('auto.features.contracts.details.sections.roomssection.dac67c0a', { defaultValue: 'No rooms are linked to this contract' })}</p>
                    <p className="text-brand-slate text-xs mt-1">{t('auto.features.contracts.details.sections.roomssection.836d12a6', { defaultValue: 'Click "Add rooms" to import rooms from the catalog' })}</p>
                </div>
            )}
            ...
            {rooms.length > 0 && (
                <div className="ring-1 ring-brand-mint sm:rounded-xl overflow-hidden">
                    <table className="w-full text-sm text-left">
                        <thead>
                            <tr className="bg-brand-light border-b border-brand-slate/20">
                                <th className="px-5 py-3 text-left text-xs font-medium text-brand-slate uppercase tracking-wider">{t('auto.features.contracts.details.sections.roomssection.700dda3b', { defaultValue: 'Room' })}</th>
                                ...
                                <th className="px-5 py-3 text-center text-xs font-medium text-brand-slate uppercase tracking-wider">{t('auto.features.contracts.details.sections.roomssection.91bb73ca', { defaultValue: 'Adults' })}</th>
                                <th className="px-5 py-3 text-center text-xs font-medium text-brand-slate uppercase tracking-wider">{t('auto.features.contracts.details.sections.roomssection.065eee1d', { defaultValue: 'Children' })}</th>
                                <th className="px-5 py-3 text-center text-xs font-medium text-brand-slate uppercase tracking-wider">{t('auto.features.contracts.details.sections.roomssection.588767e1', { defaultValue: 'Max occupancy' })}</th>
                                ...
                                <tr key={cr.id} className="hover:bg-brand-light transition-colors">
                                    <td className="px-5 py-4 whitespace-nowrap">
                                        <div className="flex flex-col">
                                            <span className="text-sm font-medium text-brand-navy">{cr.roomType?.name ?? `Room #${cr.id}`}</span>
                                    ...
                                    <td className="px-5 py-3 text-right">
                                        <button onClick={() => handleRemove(cr.id, cr.roomType?.name ?? 'chambre')}
                                            ...
                                            title={t('auto.features.contracts.details.sections.roomssection.title.4ea386bc', { defaultValue: 'Remove from contract' })}>
                                            <Trash2 size={15} />
                                        </button>
                                    </td>
                                </tr>
                    ...
                    <div className="px-5 py-3 bg-brand-light border-t border-brand-slate/20 text-xs text-brand-slate">
                        {rooms.length} contract room{rooms.length > 1 ? 's' : ''}
                    </div>
                </div>
            )}
